...
mkdir -p gasperini_eqtl_overlap
...
do
	...
	echo "GWAS_variant,region,chr,finemap_snp_intersect_grna,lower_grna_target_site,upper_grna_target_site,target_site\
,eQTL_variant,PP.H0.abf,PP.H1.abf,PP.H2.abf,PP.H3.abf,PP.H4.abf\
,nsnps,gwas_credible_set,eqtl_credible_set,SNP.PP.H4,chr.x,pos.x,a0.x,a1.x,minor_allele,minor_AF,low_confidence_variant,n_complete_samples,AC,ytx,beta.x,se,tstat,pval,Chr,Pos\
,_NUM_ID_.ss.x,rsid.x,_NUM_ID_.x,varbeta.x,z.x,chr.y,pos.y,a0.y,a1.y\
,phenotype_id,variant_id,start_distance,af,ma_samples,ma_count,pval_nominal,beta.y,slope_se,_NUM_ID_.ss.y,rsid.y,_NUM_ID_.y,varbeta.y,z.y,trait,lower_upper,method,gwas" \
> "gasperini_eqtl_overlap/${eqtl}_coloc_crispri.txt"
	...
	do
		...
		for file_num in $(seq -w 30000 10 30300); do
			...
			"gasperini_credible_sets/${file_num}/${file_num}_${chr}_stingseq_credset.txt" | \
			...
			if grep -qf temp/${file_num}_${chr}_coloc_snps.txt <(cut -d',' -f3 "temp/${file_num}_${chr}_finemap_filtered.txt"); then
	    			...
	    			"temp/${eqtl}_${file_num}_${chr}_coloc_results.txt" >> "gasperini_eqtl_overlap/${eqtl}_coloc_crispri.txt"
...
> "gasperini_eqtl_overlap/All_coloc_crispri_overlap.txt"

for j in $(seq 0 7)
do
  	eqtl="${cell_types[$j]}"

        echo "gasperini_eqtl_overlap/${eqtl}_coloc_crispri.txt"

        tail -n +2 "gasperini_eqtl_overlap/${eqtl}_coloc_crispri.txt" | \
	awk -v OFS=',' -v eqtl="$eqtl" '{print $0, eqtl}' >> "gasperini_eqtl_overlap/All_coloc_crispri_overlap.txt"

done

echo "Running GTEx intersections"

eqtl="GTEx"

echo "GWAS_variant,region,chr,finemap_snp_intersect_grna,lower_grna_target_site,upper_grna_target_site,target_site,eQTL_variant,PP.H0.abf,PP.H1.abf,PP.H2.abf,PP.H3.abf,PP.H4.abf,nsnps,gwas_credible_set,eqtl_credible_set,SNP.PP.H4,chr.x,pos.x,a0.x,a1.x,minor_allele,minor_AF,low_confidence_variant,n_complete_samples,AC,ytx,beta.x,se,tstat,pval,Chr,Pos,_NUM_ID_.ss.x,rsid.x,_NUM_ID_.x,varbeta.x,z.x,chr.y,pos.y,a0.y,a1.y,snp_hg38.x,lower_hg38,upper_hg38,rsid.ss,lower_hg19,upper_hg19,phenotype_id,variant_id,tss_distance,af,ma_samples,ma_count,pval_nominal,beta.y,slope_se,_NUM_ID_.ss.y,rsid.y,_NUM_ID_.y,varbeta.y,z.y,trait,region.x,method,gwas_name" > "gasperini_eqtl_overlap/${eqtl}_coloc_crispri.txt"
