...
fwrite(gas_finemap, "processed_data/Gasperini_gRNAs_intersecting_GWAS.txt", quote = F, row.names = F, sep = ",")
...
# What is the breakdown by study
study_n = cres_w_grnas %>%
  group_by(data) %>%
  summarise(n = n())

####################################
# Interval coloc results.          #
####################################
eqtl_w_ss_interval = fread("00.intersect_data/stingseq_eqtl_overlap/Interval_coloc_stingseq.txt") %>%
  filter(pval < 1e-05 & pval_nominal < 1e-03) %>% 
  dplyr::select(grna_target = SNP_coord, eQTL_variant = hit2, beta = beta.y, pval_nominal, PP.H4.abf,
                ensembl_id = phenotype_id, tss_distance, finemap_snp_intersect_grna, sentinel_snp = SNP, 
                gwas = gwas_x, gwas_pval = pval) %>%
  mutate(eqtl = "Interval", ensembl_id = str_split_fixed(ensembl_id, "\\.", 2)[,1]) %>%
  left_join(annot.GRC38[,c("ensembl_id","gene_name")], "ensembl_id")

eqtl_w_gas_interval = fread("00.intersect_data/gasperini_eqtl_overlap/Interval_coloc_crispri.txt") %>%
  filter(pval < 1e-05 & pval_nominal < 1e-03) %>%
  mutate(target_site = paste0(chr,":",lower_grna_target_site,"-",upper_grna_target_site)) %>%
  dplyr::select(grna_target = target_site, eQTL_variant = hit2, beta = beta.y, pval_nominal, PP.H4.abf,
                ensembl_id = phenotype_id, tss_distance, finemap_snp_intersect_grna, 
                sentinel_snp = SNP, gwas = gwas, gwas_pval = pval) %>%
  mutate(eqtl = "Interval", ensembl_id = str_split_fixed(ensembl_id, "\\.", 2)[,1]) %>%
  left_join(annot.GRC38[,c("ensembl_id","gene_name")], "ensembl_id")

eqtl_w_encode_interval = fread("00.intersect_data/encode_eqtl_overlap/Interval_coloc_crispri.txt") %>%
  filter(pval < 1e-05 & pval_nominal < 1e-03) %>% 
  unique() %>%
  mutate(target_site = paste0(chr,":",lower_grna_target_site,"-",upper_grna_target_site)) %>% 
  dplyr::select(grna_target = target_site, eQTL_variant = hit2, beta = beta.y, pval_nominal, PP.H4.abf,
...
eqtl_interval_comb = bind_rows(eqtl_w_ss_interval, eqtl_w_gas_interval, eqtl_w_encode_interval) %>%
  unique() %>% 
...
cres_w_grnas_egene = bind_rows(eqtl_cat_comb,eqtl_gtex_comb,eqtl_mage_comb,eqtl_onek_comb, eqtl_interval_comb) %>%
...
if(sum(cres_w_grnas_egene$grna_target %in% cres_w_grnas$grna_target) == nrow(cres_w_grnas_egene)){
  ...
  fwrite(cres_w_grnas_egene, "cres_with_grna_eqtls_interval.txt", quote = F, row.names = F)
} else{
...
cres_w_grnas_egene = fread("cres_with_grna_eqtls_interval.txt")
...
df1 = cres_w_grnas_egene %>%
  ...
  filter(eqtl %in% c(paste0(name,"_cells"), "GTEx", "MAGE", "Interval"))
...
interval_gene_distances = fread("eQTL_gene_distances/Interval_gene_distance.txt") %>%
  mutate(unique_id = paste0(grna_target,ensembl_id,"Interval")) %>%
  dplyr::select(unique_id, dis_rank)

gene_distances = bind_rows(onek_gene_distances,gtex_gene_distances, mage_gene_distances, interval_gene_distances)
