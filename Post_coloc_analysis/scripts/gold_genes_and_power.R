...
protein_coding_genes <- unique(gene_list$ensembl_gene_id)

write_lines(protein_coding_genes, "processed_data/ensembl_protein_coding_genes.txt")
...
fwrite(closest.genes, "processed_data/closest_gene_to_GWAS_CRE.txt", row.names = F, quote = F)
...
# Get all cis genes for null set:
...
cis_protein_coding_genes = protein_coding_genes[protein_coding_genes %in% cis.genes$ensembl_id]
writeLines(cis_protein_coding_genes, "processed_data/cis_protein_coding_genes")

# Construct Venn diagram
gold_cis_genes$gold_gene
cgenes
egenes
# install.packages(c("eulerr", "svglite")) # if needed
library(eulerr)
library(svglite)

# 1) Prepare the sets (character vectors, unique, no NAs)
gold_cis <- unique(na.omit(as.character(gold_cis_genes$gold_gene)))
crispri  <- unique(na.omit(as.character(cgenes)))
eqtl     <- unique(na.omit(as.character(egenes)))
...
# 2) Fit area-proportional Euler/Venn
fit <- euler(sets)  # warns if exact Venn isn't possible; Euler relaxes it gracefully
fit2 <- eulerr::euler(sets[c("cGenes","eGenes","Gold")])
fit3 <- eulerr::euler(sets[c("eGenes","Gold","cGenes")])
fit3 <- eulerr::euler(sets[c("Gold","eGenes","cGenes")])

# 3) Economist-style palette & styling
econ_slate <- "#FFBF00"
econ_red   <- "#d35e60"
econ_blue  <- "#76c0c1"
...
p <- plot(
  fit3,
  quantities = list(cex = 1.2, col = "#222222"),  # numbers in regions
  labels     = list(cex = 2, col = "#222222", font = 2),  # bold-ish set names
  fills      = list(fill = c(econ_slate,econ_blue, econ_red), alpha = 0.40),
  edges      = list(col  = c(econ_slate,econ_blue, econ_red), lwd = 2.5),
  adjust_labels = TRUE
)
plot(p)
# 4) Export as crisp, fully-editable SVG
svglite::svglite("plots/interval/figure_venn_econ.svg", width = 6.5, height = 6.5, bg = "white")
plot(p)
dev.off()

#-----------------------------#
# Function: Calculate enrichment of cgenes & egenes
#-----------------------------#
calculate_enrichment = function(mendel_genes){
  ...
  for (i in 1:n_iterations) {
    ...
    crandom_genes <- sample(cis_protein_coding_genes, size = length(cgenes), replace = FALSE)
    erandom_genes = sample(cis_protein_coding_genes, size = length(egenes), replace = FALSE)
  ...
  print(fisher.test(cgene_table, alternative = "two.sided"))
  ...
  print(fisher.test(egene_table, alternative = 'two.sided'))
...
#-----------------------------#
# Function: Calculate enrichment of closest GWAS genes
#-----------------------------#
closest.genes = closest.genes %>% 
  filter(grna_target %in% gold_cis_genes$grna_target)
...
calculate_enrichment = function(mendel_genes){
  ...
  cgene_overlap_counts <- numeric(n_iterations)
  egene_overlap_counts <- numeric(n_iterations)
  closest_gene_overlap_counts <- numeric(n_iterations)
  ...
  for (i in 1:n_iterations) {
    ...
    crandom_genes <- sample(protein_coding_genes, size = length(cgenes), replace = FALSE)
    erandom_genes <- sample(protein_coding_genes, size = length(egenes), replace = FALSE)
    closest_random_genes <- sample(protein_coding_genes, size = length(closest.genes$ensembl_id), replace = FALSE)
    
    # Calculate the number of overlapping genes with the Mendelian set
    cgene_overlap_counts[i] <- sum(crandom_genes %in% mendel_genes)
    egene_overlap_counts[i] <- sum(erandom_genes %in% mendel_genes)
    closest_gene_overlap_counts[i] <- sum(closest_random_genes %in% mendel_genes)
  }
  
  # Take the median of the random gene overlaps
  cgene_random_counts <- median(cgene_overlap_counts)
  egene_random_counts <- median(egene_overlap_counts)
  closest_gene_random_counts <- median(closest_gene_overlap_counts)
  
  # Observed overlaps
  cgene_observed_overlap <- sum(cgenes %in% mendel_genes)
  egene_observed_overlap <- sum(egenes %in% mendel_genes)
  closest_gene_observed_overlap <- sum(closest.genes$ensembl_id %in% mendel_genes)
  ...
  # closest genes
  a2 <- closest_gene_observed_overlap
  b2 <- length(closest.genes$ensembl_id) - a2
  c2 <- closest_gene_random_counts
  d2 <- length(closest.genes$ensembl_id) - c2
  closest_gene_table <- matrix(c(a2, b2, c2, d2), nrow = 2)
  print("closest_gene_table:")
  print(closest_gene_table)
  print(fisher.test(closest_gene_table, alternative = "greater"))
  ...
  print("Fisher test comparing cgenes vs egenes:")
  print(fisher.test(matrix(c(a, b, a1, b1), nrow = 2)))
  print("Fisher test comparing cgenes vs closest_genes:")
  print(fisher.test(matrix(c(a, b, a2, b2), nrow = 2)))
  print("Fisher test comparing egenes vs closest_genes:")
  print(fisher.test(matrix(c(a1, b1, a2, b2), nrow = 2)))
}
...
#-----------------------------#
# Function: Calculate enrichment of ABC GWAS genes
#-----------------------------#
...
cres_w_grnas_abc = fread("ABC/cres_w_grnas_ABC_intersections.txt")[,-5] %>% 
  ...
  arrange(desc(ABC.Score)) %>% # keep gene with highest ABC score
  distinct(target_site, .keep_all = T) %>%
  filter(target_site %in% gold_genes$grna) # filter to grnas within 1mbp of gold genes
...
calculate_enrichment = function(mendel_genes){
  ...
  cgene_overlap_counts <- numeric(n_iterations)
  egene_overlap_counts <- numeric(n_iterations)
  abc_overlap_counts <- numeric(n_iterations)
  ...
  for (i in 1:n_iterations) {
    ...
    crandom_genes <- sample(protein_coding_genes, size = length(cgenes), replace = FALSE)
    erandom_genes <- sample(protein_coding_genes, size = length(egenes), replace = FALSE)
    abc_random_genes <- sample(protein_coding_genes, size = length(cres_w_grnas_abc$ensembl_id), replace = FALSE)
    
    # Calculate the number of overlapping genes with the Mendelian set
    cgene_overlap_counts[i] <- sum(crandom_genes %in% mendel_genes)
    egene_overlap_counts[i] <- sum(erandom_genes %in% mendel_genes)
    abc_overlap_counts[i] <- sum(abc_random_genes %in% mendel_genes)
  }
  
  # Take the median of the random gene overlaps
  cgene_random_counts <- median(cgene_overlap_counts)
  egene_random_counts <- median(egene_overlap_counts)
  abc_random_counts <- median(abc_overlap_counts)
  
  # Observed overlaps
  cgene_observed_overlap <- sum(cgenes %in% mendel_genes)
  egene_observed_overlap <- sum(egenes %in% mendel_genes)
  abc_observed_overlap <- sum(cres_w_grnas_abc$ensembl_id %in% mendel_genes)
  ...
  # closest genes
  a2 <- abc_observed_overlap
  b2 <- length(cres_w_grnas_abc$ensembl_id) - a2
  c2 <- abc_random_counts
  d2 <- length(cres_w_grnas_abc$ensembl_id) - c2
  closest_gene_table <- matrix(c(a2, b2, c2, d2), nrow = 2)
  print("abc_gene_table:")
  print(closest_gene_table)
  print(fisher.test(closest_gene_table, alternative = "greater"))
  ...
  print("Fisher test comparing cgenes vs egenes:")
  print(fisher.test(matrix(c(a, b, a1, b1), nrow = 2)))
  print("Fisher test comparing cgenes vs abc genes:")
  print(fisher.test(matrix(c(a, b, a2, b2), nrow = 2)))
  print("Fisher test comparing egenes vs abc genes:")
  print(fisher.test(matrix(c(a1, b1, a2, b2), nrow = 2)))
}
...
#-----------------------------#
data <- data.frame(
  Gene_type = factor(c("cGenes", "ABC-Max","eGenes", "Closest gene", "Hi-C"), levels = rev(c("cGenes","ABC-Max", "eGenes", "Closest gene", "Hi-C"))),
  Proportion = c(23 / length(cgenes), 37/ length(cres_w_grnas_abc$ensembl_id), 65 / length(egenes), 91/ length(closest.genes$ensembl_id), 34 / length(hic_genes)),
  N = c(length(cgenes), length(cres_w_grnas_abc$ensembl_id), length(egenes), length(closest.genes$ensembl_id),length(hic_genes))
)

data$standard_error = sqrt(data$Proportion * (1 - data$Proportion) / data$N)
...
svg("plots/figure_plots/gold_standard_genes_hic_barplots.svg", width = 4.5, height = 6)

ggplot(data, aes(x = Gene_type, y = Proportion, fill = Gene_type)) +
  geom_bar(stat = "identity", color = "black", width = 0.5) +
  geom_errorbar(aes(ymin = Proportion - standard_error, ymax = Proportion + standard_error), 
...
expressed = gold_cis_genes$gold_gene[gold_cis_genes$gold_gene %in% k562_genes] # expressed genes
not_expressed = gold_cis_genes$gold_gene[!gold_cis_genes$gold_gene %in% k562_genes] # not expressed genes
underpowered = gold_cis_genes$gold_gene[gold_cis_genes$gold_gene %in% crispri.power$gene] # underpowered
underpowered = underpowered[!underpowered %in% not_expressed]

length(expressed)/length(gold_cis_genes$gold_gene)
length(not_expressed)/length(gold_cis_genes$gold_gene)
length(underpowered)/length(gold_cis_genes$gold_gene)

#--------------------------------------#
# Process & combine eQTL power data    #
#--------------------------------------#
power.files = system("ls power_results/power.results.beta0.27*", intern = T)
...
fwrite(eqtl.power, "power_results/eqtl.power.combined.interval.txt", quote = F, row.names = F, col.names = T)

# sort by power and remove duplicated
eqtl.under.powered = eqtl.power %>% 
  arrange(desc(eQTL.power)) %>% 
  distinct(ensembl_id, .keep_all=T) %>%
  filter(eQTL.power < 0.8)

expressed = gold_cis_genes$gold_gene[gold_cis_genes$gold_gene %in% eqtl.power$ensembl_id] # expressed genes
not_expressed = gold_cis_genes$gold_gene[!gold_cis_genes$gold_gene %in% eqtl.power$ensembl_id] # not expressed genes
underpowered = gold_cis_genes$gold_gene[gold_cis_genes$gold_gene %in% eqtl.under.powered$ensembl_id] # underpowered
underpowered = underpowered[!underpowered %in% not_expressed]
...
gene_grid <- gold_cis_genes %>%
  mutate(
    ...
    crispri_underpowered = (gold_gene %in% crispri.power$gene | !gold_cis_genes$gold_gene %in% k562_genes),
    eqtl_underpowered = gold_gene %in% eqtl.under.powered$ensembl_id 
  )
...
cat("number of gold loci with multiple genes",sum(data$egene_multiple > 0)/nrow(data))

cat("number of gold loci with multiple genes",sum(data$cgene_multiple > 0)/nrow(data))

#### USE table to make waffle plot ####
table = data %>%
...
cat("total squares", sum(apply(table[,2:5], 2, sum)))

cat("total cgene squares",sum(apply(table[c(1,2,5),2:5], 2, sum)))

cat("total egene squares",sum(apply(table[c(3,4,5),2:5], 2, sum)))

### Following code is for the power venn diagrams ####
...
cgene_unpow = sum((cgene_venn$crispri_underpowered) & cgene_venn$cgene_multiple == 0)
...
# underpowered/not expressed
egene_unpow = sum((egene_venn$eqtl_underpowered) & egene_venn$egene_multiple == 0)
# another gene in locus & underpowered
intersection = sum(egene_venn$egene_multiple > 0 & egene_venn$eqtl_underpowered == T)
