...
mm <- model.matrix(
  ~ scaled_mean_PP.H4 + Episcore + I(abs(tss_distance_100kbp)) +
    factor(gold_standard) + median_tpm + eqtl_log_pvalue +
    mean_gwas_pval + pLI + num_enh,
  data = regression_df
)
colSums(!is.finite(mm))        # which predictors have non-finite values?
which(!is.finite(regression_df$gene_intersect))  # response non-finite?
...
overlapping_eqtls %>%
  ...
  mutate(eqtl = factor(eqtl, levels = unique(eqtl)),  # Use unique values for factor levels
         group = ifelse(eqtl %in% c("blood", "GTEx","Interval"), "Bulk_tissue_eQTL", 
...
venn_df <- overlapping_eqtls %>%
  mutate(eqtl = ifelse(eqtl == "GTEx", "blood", eqtl)) %>%
  mutate(group = ifelse(eqtl %in% c("blood", "GTEx","Interval"), "Bulk_tissue_eQTL", 
                        ifelse(eqtl %in% sc_qtls, "Sc_eQTL", "Bulk_cell_type_eQTL")),
...
test = wilcox.test(plot_df2$GTEx_power[plot_df2$unique_CREs == "cGene"], 
                   plot_df2$GTEx_power[plot_df2$unique_CREs == "eGene"])
print(test)
