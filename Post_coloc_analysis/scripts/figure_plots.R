...
# proportion of cgenes within 100kb
df1$prop[df1$bins == 100] + df1$prop[df1$bins == -100]
# proportion of cgenes within 100kb
...
svg("plots/interval/tss_distance_barplots.svg", width = 8, height = 8)
...
svg("plots/interval/tss_distance_100kb.svg", width = 8, height = 8)
...
gene_lengths = fread("data/gene_length.txt")

egene_length = gene_lengths$length[gene_lengths$ensembl_id %in% egenes]
cgene_length = gene_lengths$length[gene_lengths$ensembl_id %in% cgenes]

### Plot number of enhancers ###
# Extract cgene_metric
cgene_metric = gene_metrics[ensemble_id %in% cgenes, c("ensemble_id","num_enh","median_tpm")] %>%
  left_join(gene_lengths, by = c("ensemble_id" = "ensembl_id")) %>%
  filter(is.na(num_enh) ==F) %>%
  mutate(group = as.factor("cGenes"))

egene_metric = gene_metrics[ensemble_id %in% egenes, c("ensemble_id","num_enh","median_tpm")] %>%
  left_join(gene_lengths, by = c("ensemble_id" = "ensembl_id")) %>%
  filter(is.na(num_enh) ==F) %>%
  mutate(group = as.factor("eGenes"))

df = bind_rows(cgene_metric, egene_metric)

fit <- glm(num_enh ~ group + log1p(length), family = "poisson",data = df)
summary(fit)
coef_summary <- summary(fit)$coefficients
pvals <- coef_summary[, "Pr(>|z|)"]
...
svg("plots/interval/enhancer_plot.svg", width = 4, height = 6)

enhancer_plot <- df %>%
  filter(num_enh<130) %>% #improve plot look
  ggplot(aes(x = group, y = num_enh, fill = group)) +
  ...
  theme_cowplot() +
  annotate("text", x = 1.5, y = 175, label = paste0("P = ", formatC(pvals[2], format = "e", digits = 1)), size = 5, hjust = 0.5, family = "sans") +
...
### test number of super enhancers ###
# Extract cgene_metric
cgene_metric = gene_metrics[ensemble_id %in% cgenes, c("ensemble_id","num_super_enh","median_tpm")] %>%
  left_join(gene_lengths, by = c("ensemble_id" = "ensembl_id")) %>%
  filter(is.na(num_super_enh) ==F) %>%
  mutate(group = as.factor("cGenes"))

egene_metric = gene_metrics[ensemble_id %in% egenes, c("ensemble_id","num_super_enh","median_tpm")] %>%
  left_join(gene_lengths, by = c("ensemble_id" = "ensembl_id")) %>%
  filter(is.na(num_super_enh) ==F) %>%
  mutate(group = as.factor("eGenes"))
...
fit <- glm(num_super_enh ~ group + log1p(length), family = "poisson",data = df)
summary(fit)
coef_summary <- summary(fit)$coefficients
pvals <- coef_summary[, "Pr(>|z|)"]
...
### Plot pLI ###
# Extract cgene_metric
cgene_metric = gene_metrics[ensemble_id %in% cgenes, c("ensemble_id","pLI","median_tpm")] %>%
  left_join(gene_lengths, by = c("ensemble_id" = "ensembl_id")) %>%
  filter(is.na(pLI) ==F) %>%
  mutate(group = as.factor("cGenes"))

egene_metric = gene_metrics[ensemble_id %in% egenes, c("ensemble_id","pLI","median_tpm")] %>%
  left_join(gene_lengths, by = c("ensemble_id" = "ensembl_id")) %>%
  filter(is.na(pLI) ==F) %>%
  mutate(group = as.factor("eGenes"))

df = bind_rows(cgene_metric, egene_metric) %>%
  mutate(pLI = ifelse(pLI > 0.9, 1, 0))

fit <- glm(pLI ~ group + log1p(length), family = "binomial",data = df)
summary(fit)
coef_summary <- summary(fit)$coefficients
pvals <- coef_summary[, "Pr(>|z|)"]
...
data <- data.frame(
  ...
  Proportion = c(sum(cgene_metric$pLI > 0.9)/length(cgene_metric$pLI),sum(egene_metric$pLI > 0.9)/length(egene_metric$pLI)))
data
#png("plots/figure_plots/pli_plot.png", width = 4, height = 6,units = "in", res = 300)
svg("plots/interval/pli_plot.svg", width = 4, height = 6)
...
pli_plot = ggplot(data, aes(x = Gene_type, y = Proportion, fill = Gene_type)) +
  ...
  annotate("text", x = 1.5, y = 0.35, label = paste0("P = ", formatC(pvals[2], format = "e", digits = 1)), size = 5, hjust = 0.5, family = "sans") +
  scale_fill_manual(values = colors) +
  theme_cowplot() +
  theme(
    axis.title.x = element_text(size = 16),
...
### Plot Episcore ###
# Extract cgene_metric
cgene_metric = gene_metrics[ensemble_id %in% cgenes, c("ensemble_id","Episcore")] %>%
  left_join(gene_lengths, by = c("ensemble_id" = "ensembl_id")) %>%
  filter(is.na(Episcore) ==F) %>%
  mutate(group = as.factor("cGenes"))

egene_metric = gene_metrics[ensemble_id %in% egenes, c("ensemble_id","Episcore")] %>%
  left_join(gene_lengths, by = c("ensemble_id" = "ensembl_id")) %>%
  filter(is.na(Episcore) ==F) %>%
  mutate(group = as.factor("eGenes"))
...
fit <- glm(Episcore ~ group + log1p(length), family = "gaussian",data = df)
summary(fit)
coef_summary <- summary(fit)$coefficients
pvals <- coef_summary[, "Pr(>|t|)"]

#png("plots/figure_plots/enhancer_plot.png", width = 4, height = 6,units = "in", res = 300)
svg("plots/interval/Haploinsuficiency_plot.svg", width = 4, height = 6)

enhancer_plot <- df %>%
  ggplot(aes(x = group, y = Episcore, fill = group)) +
  geom_violin(trim = FALSE, fill = "light grey", color = "black", alpha = 0.5) +  # Violin plot
  geom_boxplot(width = 0.1, color = "black", outlier.shape = NA) +  # Boxplot inside violin plot
  scale_fill_manual(values = colors) +
  theme_cowplot() +
  annotate("text", x = 1.5, y = 0.99, label = paste0("P = ", formatC(pvals[2], format = "e", digits = 1)), size = 5, hjust = 0.5, family = "sans") +
...
### test pHaplo ###
# Extract cgene_metric
cgene_metric = gene_metrics[ensemble_id %in% cgenes, c("ensemble_id","pHaplo")] %>%
  left_join(gene_lengths, by = c("ensemble_id" = "ensembl_id")) %>%
  filter(is.na(pHaplo) ==F) %>%
  mutate(group = as.factor("cGenes"))

egene_metric = gene_metrics[ensemble_id %in% egenes, c("ensemble_id","pHaplo")] %>%
  left_join(gene_lengths, by = c("ensemble_id" = "ensembl_id")) %>%
  filter(is.na(pHaplo) ==F) %>%
  mutate(group = as.factor("eGenes"))
...
fit <- glm(pHaplo ~ group + log1p(length), family = "gaussian",data = df)
summary(fit)
coef_summary <- summary(fit)$coefficients
pvals <- coef_summary[, "Pr(>|t|)"]
...
cgene_metric = K562_bulk[ensembl_gene_id %in% cgenes] %>%
  left_join(gene_lengths, by = c("ensembl_gene_id" = "ensembl_id")) %>%
  filter(is.na(TPM) ==F) %>%
  mutate(group = as.factor("cGenes"))

egene_metric = K562_bulk[ensembl_gene_id %in% egenes] %>%
  left_join(gene_lengths, by = c("ensembl_gene_id" = "ensembl_id")) %>%
  filter(is.na(TPM) ==F) %>%
  mutate(group = as.factor("eGenes"))
...
fit <- glm(log2(TPM+1) ~ group + log1p(length), family = "gaussian", data = df)
summary(fit)
coef_summary <- summary(fit)$coefficients
pvals <- coef_summary[, "Pr(>|t|)"]

#png("plots/figure_plots/k562_ex_plot.png", width = 4, height = 6,units = "in", res = 300)
svg("plots/interval/k562_ex_plot.svg", width = 4, height = 6)

# K562 expression violin plot
k562_ex_plot <- df %>%
  filter(TPM<80) %>%
  ggplot(aes(x = group, y = TPM, fill = group)) +
  geom_violin(trim = FALSE, fill = "light grey", color = "black", alpha = 0.5) +  # Violin plot
  geom_boxplot(width = 0.1, color = "black", outlier.shape = NA) +  # Boxplot inside violin plot
  scale_fill_manual(values = colors) +
  theme_cowplot() +
  ylim(0,80) +
  annotate("text", x = 1.5, y = 80, label = paste0("P = ", formatC(pvals[2], format = "e", digits = 1)), size = 5, hjust = 0.5, family = "sans") +
...
colnames(blood_tpm) = c("ensembl_id", "TPM")

cgene_metric = blood_tpm[ensembl_id %in% cgenes] %>%
  left_join(gene_lengths, by = c("ensembl_id" = "ensembl_id")) %>%
  filter(is.na(TPM) ==F) %>%
  mutate(group = as.factor("cGenes"))

egene_metric = blood_tpm[ensembl_id %in% egenes] %>%
  left_join(gene_lengths, by = c("ensembl_id" = "ensembl_id")) %>%
  filter(is.na(TPM) ==F) %>%
  mutate(group = as.factor("eGenes"))
...
svg("plots/interval/blood_ex_plot.svg", width = 4, height = 6)
...
svg("plots/interval/closest_barplots.svg", width = 4, height = 6)
...
svg("plots/interval/num_genes_barplot.svg", width = 4, height = 6)
...
venn_df <- overlapping_eqtls %>%
  ...
  mutate(group = ifelse(eqtl %in% c("blood", "GTEx"), "GTEx, Twins UK blood", 
                        ifelse(eqtl == "Interval", "Interval blood",
...
upset_data <- venn_df %>%
  dplyr::select(target_gene, group) %>%
  distinct() %>%
...
svg(filename = "plots/interval/upset_plot_interval.svg", width = 10, height = 8)

# Step 3: Create the UpSet Plot
upset_plot = upset(upset_matrix, 
      sets = c("GTEx, Twins UK blood","Interval blood", "Sc eQTL", "Bulk cell type eQTL"), 
      order.by = "freq", 
      decreasing = T,
      keep.order = FALSE, 
      main.bar.color = "black", 
      sets.bar.color = rev(c("#01a2d9","#6794a7","#6794a7","#014d64")),
...
# Which ancestry does each variant belong?
sting_seq = fread("CRISPR_data/All_STING_seq_CREs.csv") %>%
  rename(SS_coord = `SNP Coordinates (hg19)`)

maf_plot = cres_w_grnas %>%
  mutate(population = ifelse(grna_target %in% sting_seq$SS_coord[sting_seq$Population %in% c("Multi-ancestry","EA","EAS","SA","AA","HA")], "multi_ancestry","european"))

# Extract European MAFs
...
sumstats_filtered = sumstats %>% filter(snp_pos %in% unique(maf_plot$snp_pos[maf_plot$population == "european"]))

# Extract multi-ancestry MAFs
setDTthreads(4)
multi_sumstat <- fread("/gpfs/commons/groups/lappalainen_lab/sghatan/stingseq_eqtl_overlap/data/BCX_sumstats/BAS_filter.tsv",  nThread = getDTthreads())
multi_sumstat$snp_pos = str_split_fixed(multi_sumstat$rs_number,"_",2)[,1]
multi_sumstat_filtered = multi_sumstat %>% filter(snp_pos %in% unique(maf_plot$snp_pos[maf_plot$population != "european"]))
fwrite(multi_sumstat_filtered, "processed_data/multi_sumstat_filtered.txt", row.names = F, quote = F)
...
mafs <- merge(
  sumstats_filtered[, .(snp_pos, minor_AF)],
  multi_sumstat_filtered[, .(snp_pos, eaf)],
  by = "snp_pos", all = TRUE
)[, maf := fifelse(!is.na(minor_AF), minor_AF, pmin(eaf, 1 - eaf))
][, .(snp_pos, maf)]
...
plot_df = maf_plot %>%
  ...
  left_join(mafs, "snp_pos") %>%
  distinct(snp_pos, .keep_all = T)

fit <- glm(maf ~ unique_CREs + as.factor(population), family = "gaussian", data = plot_df)
summary(fit)
coef_summary <- summary(fit)$coefficients
pvals <- coef_summary[, "Pr(>|t|)"]

#png("plots/figure_plots/boxplot_maf.png", width = 4, height = 6,units = "in", res = 300)
svg("plots/interval/boxplot_maf.svg", width = 4, height = 6)

boxplot_maf = plot_df %>%
  filter(maf < 0.50) %>%
  ggplot(aes(x = unique_CREs, y = maf, fill = unique_CREs)) +
  geom_violin(trim = FALSE, fill = "light grey", color = "black", alpha = 0.5) +  # Violin plot
  geom_boxplot(width = 0.1, color = "black", outlier.shape = NA) +  # Boxplot inside violin plot
  scale_fill_manual(values = colors) +
  theme_cowplot() +
  ylim(0,0.5) +
  annotate("text", x = 1.5, y = 0.51, label = paste0("P = ",formatC(pvals[2], format = "e", digits = 1)), size = 5, hjust = 0.5) +
...
pp.h4.df = cres_w_grnas_egene %>%
  ...
  mutate(mean_PP.H4 = mean(PP.H4.abf,na.rm=T)) %>% # Take the average PP.H4 from multiple GWAS
  mutate(max_PP.H4 = max(PP.H4.abf,na.rm=T)) %>%
  mutate(scaled_mean_PP.H4 = mean_PP.H4*2) %>% # scale pp.h4 so that 0.5 is one unit change
  mutate(scaled_max_PP.H4 = max_PP.H4*2) %>%
  distinct(target_gene,.keep_all=T) %>%
  ungroup()

test = wilcox.test(pp.h4.df$max_PP.H4[pp.h4.df$gene_intersect==1], pp.h4.df$max_PP.H4[pp.h4.df$gene_intersect == 0])
print(test)

pp.h4.plot = pp.h4.df %>% 
  mutate(gene_intersect = ifelse(gene_intersect == 1, "eQTL with\ncGene", "eQTL without\ncGene")) %>%
  ggplot(aes(x = gene_intersect, y = max_PP.H4, fill = gene_intersect)) +
  geom_violin(trim = FALSE, fill = "light grey", color = "black", alpha = 0.5) +  # Violin plot
  geom_boxplot(width = 0.1, color = "black", outlier.shape = NA) +  # Boxplot inside violin plot
  theme_cowplot() +
...
svg("plots/interval/pp.h4_violin_plot.svg", width = 4, height = 6)
pp.h4.plot
dev.off()
...
eqtl.power = fread("power_results/eqtl.power.combined.interval.txt") %>%
  filter(snp != "snp") %>%
  mutate(target_gene = paste0(grna_target,"_",ensembl_id), 
         eQTL.power = as.numeric(eQTL.power)) %>%
...
svg("plots/interval/crispri_power_barplot.svg", width = 4, height = 6)
...
ggplot(data, aes(x = Gene_type, y = Proportion)) +
  ...
  annotate("text", x = 1.5, y = 0.99, label = paste0("P = ", formatC(test$p.value, format = "e", digits = 1)), size = 5, hjust = 0.5, family = "sans") +
  theme_cowplot() +
  ylim(c(0,1)) +
  theme(
    axis.title.x = element_text(size = 16),
...
# load interval protein coding genes
gene_list <- read_excel("data/interval_genes.xlsx",sheet = 1) %>%
  filter(gene_biotype == "protein_coding")

eqtl.power.overlap = eqtl.power %>%
  filter(ensembl_id %in% gene_list$feature_id) %>%
  distinct(target_gene, .keep_all=T) %>%
  filter(target_gene %in% c(overlapping_cres_egene,overlapping_cres_cgene)) %>%
  mutate(intersect = as.factor(ifelse(target_gene %in% overlapping_target_genes, "Intersection", "No intersection")))
...
svg("plots/interval/eqtl_power_barplot.svg", width = 4, height = 6)
