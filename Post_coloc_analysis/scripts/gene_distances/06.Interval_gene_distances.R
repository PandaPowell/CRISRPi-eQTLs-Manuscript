...
process_coloc_data = function(coloc_data, sum_stats_list) {
    ...
    for (i in 1:nrow(coloc_data)) {
      ...
      if (!chr %in% names(sum_stats_list)) {
        eqtl_dir = paste0("/gpfs/commons/datasets/controlled/INTERVAL/public_sumstats/cis/INTERVAL_eQTL_summary_statistics/INTERVAL_eQTL_nominal_chr",chr,".tsv")
        sum_stats_list[[chr]] = fread(eqtl_dir) %>%
          mutate(snp_hg19 = paste(gsub("chr", "", chr), pos_b37, effect_allele, other_allele, sep = ":"))
      }
...
fwrite(all_res2,"eQTL_gene_distances/Interval_gene_distance.txt",quote = F, row.names = F)
