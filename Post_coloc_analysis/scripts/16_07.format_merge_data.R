...
resample_results = read.fst("CRISPR_data/resampling_results.fst") %>% unique() %>% 
...
# all_genes = annot.GRC37 %>% left_join(annot.GRC38, "ensembl_id") %>%
#   dplyr::select(chr = V1.y, GRC37_tss = end.x, GRC38_tss = end.y, ensembl_id, gene_name = gene_name.y)
...
ss_finemap = fread("Intersect_data/sting_seq_credible_sets/merged_stingseq_credset.txt") %>%
  ...
  dplyr::select(grna_target = SS_coord, logfc = `Log2 fold-change`,significant, ensembl_id = `Ensembl ID`, gene_name = Gene,
...
ss_finemap_bcx = fread("Intersect_data/sting_seq_credible_sets/BCX/merged_credset.txt") %>%
  rename(SS_coord = SNP_coord) %>%
  left_join(sting_seq,"SS_coord") %>%
...
gas_finemap = fread("Intersect_data/gasperini_credible_sets/merged_gasperini_credset_closest.txt") %>%
  ...
  mutate(grna_target = paste0(chr.x, ":", lower_grna_target_site,"-",upper_grna_target_site),
         ...
         tss_distance = TSS-upper_grna_target_site,
         grna_pos = round((lower_grna_target_site+upper_grna_target_site)/2)) %>%
  dplyr::select(grna_target, logfc = xi, significant, ensembl_id = gene_id, gene_name = gene_short_name,
                tss_distance, finemap_snp_intersect_grna, sentinel_snp = finemap_snp, gwas, data, grna_pos)

gas_finemap_bcx = fread("Intersect_data/gasperini_credible_sets/BCX/merged_credset.txt") %>%
  left_join(resample_results, "target_site") %>%
  distinct(pair_id, .keep_all=T) %>%
  mutate(grna_target = paste0(chr.x, ":", lower_grna_target_site,"-",upper_grna_target_site),
         significant = as.integer(rejected),
         tss_distance = TSS-upper_grna_target_site, data = "gasperini",
         grna_pos = round((lower_grna_target_site+upper_grna_target_site)/2)) %>%
  dplyr::select(grna_target, logfc = xi, significant, ensembl_id = gene_id, gene_name = gene_short_name,
                tss_distance, finemap_snp_intersect_grna, sentinel_snp = finemap_snp, gwas, data, grna_pos)

encode_finemap = fread("Intersect_data/encode_credible_sets/merged_encode_credset.txt") %>%
  mutate(target_site = paste0(chr,":",lower_grna_target_site,"-",upper_grna_target_site)) %>%
  left_join(encode, "target_site") %>%
  distinct(name, .keep_all = T) %>%
  mutate(tss_distance = startTSS-chromEnd_GRC38) %>%
  mutate(gwas = as.character(gwas), data = "encode", grna_pos = round((lower_grna_target_site+upper_grna_target_site)/2)) %>%
  dplyr::select(grna_target = target_site, logfc = Effectsize, significant = Significant, ensembl_id, 
                gene_name,tss_distance, finemap_snp_intersect_grna, sentinel_snp = finemap_snp, gwas, data, grna_pos)
  
encode_finemap_bcx = fread("Intersect_data/encode_credible_sets/BCX/merged_credset.txt") %>%
  mutate(target_site = paste0(chr,":",lower_grna_target_site,"-",upper_grna_target_site)) %>%
  left_join(encode, "target_site") %>%
  distinct(name, .keep_all = T) %>%
  mutate(tss_distance = startTSS-chromEnd_GRC38, gwas = as.character(gwas), 
...
# Check if gRNA positions intersect
for (i in 1:22){
...
####################################
eqtl_w_ss_cat = fread("Intersect_data/stingseq_eqtl_overlap/eqtl_catalogue_coloc_stingseq.txt") %>%
...
eqtl_w_ss_cat[is.na(gene_name),]

eqtl_w_gas_cat = fread("Intersect_data/gasperini_eqtl_overlap/eqtl_catalogue_coloc_gasperini.txt") %>%
  mutate(target_site = paste0(chr,":",lower_grna_target_site,"-",upper_grna_target_site)) %>%
...
eqtl_w_encode_cat = fread("Intersect_data/encode_eqtl_overlap/eqtl_catalogue_coloc_crispri.txt") %>%
  mutate(target_site = paste0(chr,":",lower_grna_target_site,"-",upper_grna_target_site)) %>%
  dplyr::select(grna_target = target_site, eQTL_variant = eqtl_hit_hg19, PP.H4.abf,
                ensembl_id = molecular_id, finemap_snp_intersect_grna, sentinel_snp = GWAS_variant,
...
eqtl_cat_comb = bind_rows(split1,split2)

# Load cis-genes and calculate distance rank
cat_sumstats = fread("eQTL_gene_distances/eqtl_cat_gene_distances.txt") %>%
  mutate(ensembl_id = molecular_trait_id, unique_id = paste(grna_target,ensembl_id,dataset_id, sep = "_")) %>%
  left_join(annot.GRC38, "ensembl_id") %>%
  filter(is.na(start) == F) %>%
  mutate(grna_target_dataset = paste0(grna_target,"_",dataset_id), 
         tss_distance = start-position) %>%
...
fwrite(cat_sumstats, "eQTL_gene_distances/eqtl_cat_gene_distances_formatted.txt", quote=F, row.names = F)

eqtl_cat_comb = eqtl_cat_comb %>% 
...
# GTEx coloc results.             ##
####################################
eqtl_w_ss_gtex =  fread("Intersect_data/stingseq_eqtl_overlap/GTEx_coloc_stingseq.txt") %>%
...
eqtl_w_gas_gtex = fread("Intersect_data/gasperini_eqtl_overlap/GTEx_coloc_crispri.txt") %>%
  filter(pval < 1e-05 & pval_nominal < 1e-03) %>%
  mutate(target_site = paste0(chr,":",lower_grna_target_site,"-",upper_grna_target_site)) %>%
...
eqtl_w_encode_gtex = fread("Intersect_data/encode_eqtl_overlap/GTEx_coloc_crispri.txt") %>%
  filter(pval < 1e-05 & pval_nominal < 1e-03) %>% 
  unique() %>%
  mutate(target_site = paste0(chr,":",lower_grna_target_site,"-",upper_grna_target_site)) %>% 
  dplyr::select(grna_target = target_site, eQTL_variant, beta = beta.y, pval_nominal, PP.H4.abf,
...
gtex_gene_distances = fread("eQTL_gene_distances/GTEx_gene_distance.txt") %>%
  mutate(coord = str_extract(GWAS_variant, "[0-9]\\:[0-9]+(?=\\:[A-Z])"),
         ensembl_id = str_split_fixed(phenotype_id, "\\.",2)[,1],
         unique_id = paste0(target_site,ensembl_id))

eqtl_gtex_comb = eqtl_gtex_comb %>% 
  left_join(gtex_gene_distances[,c("unique_id","eqtl_variant","dis_rank")], "unique_id") %>%
  rename(eqtl_hit_hg38 = eqtl_variant)

####################################
# MAGE coloc results.              #
####################################
eqtl_w_ss_mage =  fread("Intersect_data/stingseq_eqtl_overlap/MAGE_coloc_stingseq.txt") %>%
...
eqtl_w_gas_mage = fread("Intersect_data/gasperini_eqtl_overlap/MAGE_coloc_gasperini.txt") %>%
  filter(`p-value` < 1e-05 & pval_nominal < 1e-03) %>% 
  mutate(gwas = as.character(gwas)) %>%
  mutate(target_site = paste0(chr,":",lower_grna_target_site,"-",upper_grna_target_site)) %>%
...
eqtl_w_encode_mage = fread("Intersect_data/encode_eqtl_overlap/MAGE_coloc_crispri.txt") %>%
  filter(`p-value` < 1e-05 & pval_nominal < 1e-03) %>%
...
eqtl_mage_comb = eqtl_mage_comb %>% 
  left_join(mage_gene_distances[,c("unique_id","eqtl_variant","dis_rank")], "unique_id") %>%
  rename(eqtl_hit_hg38 = eqtl_variant)

####################################
# OneK1K coloc results.    #
####################################
eqtl_w_ss_1k = fread("Intersect_data/stingseq_eqtl_overlap/All_coloc_stingseq_overlap.txt") %>%
  filter(pval < 1e-05 & pval_nominal < 1e-03) %>% 
  mutate(gwas = as.character(gwas)) %>%
  dplyr::select(grna_target = SS_coord, eQTL_variant, beta = beta.y, pval_nominal, PP.H4.abf,
...
eqtl_w_gas_1k = fread("Intersect_data/gasperini_eqtl_overlap/All_coloc_crispri_overlap.txt") %>%
  filter(pval < 1e-05 & pval_nominal < 1e-03) %>%
  mutate(target_site = paste0(chr,":",lower_grna_target_site,"-",upper_grna_target_site)) %>%
  dplyr::select(grna_target = target_site, eQTL_variant, beta = beta.y, pval_nominal, PP.H4.abf,
                gene_name = phenotype_id, tss_distance = start_distance, finemap_snp_intersect_grna, 
...
eqtl_w_encode_1k = fread("Intersect_data/encode_eqtl_overlap/All_coloc_crispri_overlap.txt") %>%
  filter(pval < 1e-05 & pval_nominal < 1e-03) %>%
  mutate(target_site = paste0(chr,":",lower_grna_target_site,"-",upper_grna_target_site)) %>%
  dplyr::select(grna_target = target_site, eQTL_variant, beta = beta.y, pval_nominal, PP.H4.abf,
                gene_name = phenotype_id, tss_distance = start_distance, finemap_snp_intersect_grna,
...
fwrite(onek_gene_distances, "eQTL_gene_distances/Onek1k_gene_distance_formatted.txt", quote=F, row.names = F)
...
eqtl_onek_comb = bind_rows(eqtl_w_ss_1k, eqtl_w_gas_1k, eqtl_w_encode_1k) %>% unique() %>%
  ...
  left_join(onek_gene_distances[,c("distance","unique_id","dis_rank")], "unique_id") %>%
  unique() %>% dplyr::select(-tss_distance) %>% rename(tss_distance = distance) %>%
  left_join(annot.GRC38, "gene_name")
...
cres_w_grnas_egene = bind_rows(eqtl_cat_comb,eqtl_gtex_comb,eqtl_mage_comb,eqtl_onek_comb) %>%
  dplyr::select("grna_target", "eQTL_variant", "PP.H4.abf", "ensembl_id", "gene_name",
         ...
         "dataset_id", "beta", "pval_nominal", "gwas_pval", "median_tpm", 
         "dis_rank") %>% 
  left_join(merged_sites, "grna_target") %>%
  dplyr::select(-grna_target) %>% 
  rename(grna_target = grna_target_merge) %>%
  filter(is.na(grna_target) == F) %>%
  mutate(target_gene = ifelse(is.na(gene_name) == F,paste0(grna_target,"_", gene_name),paste0(grna_target,"_", ensembl_id))) %>%
...
cres_w_grnas_egene[is.na(gene_name),]
...
trans_genes = c("NFE2","GFI1B","IKZF1", "HHEX","RUNX1")

View(cres_w_grnas_egene[target_gene %in% overlapping_target_genes & gene_name %in% trans_genes])
