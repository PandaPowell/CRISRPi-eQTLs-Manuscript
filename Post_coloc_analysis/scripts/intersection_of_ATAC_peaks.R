...
png("plots/interval/ATACseq_intersections.png", width = 6, height = 4,units = "in", res = 300)
...
atac_overlap = fread("ATAC_overlap/intersecting_ATAC_peaks_w_cres.bed", fill = T) %>%
  mutate(cell = str_split_fixed(cell_peak,"_",3)[,1], cre_size = upper-lower) %>%
  distinct(gRNA_target, cell_peak, .keep_all = T)
...
atac_counts = atac_overlap %>%
  ...
  summarise(n = n(), cre_size = cre_size)
...
egene_cgene_atac = atac_counts %>%
  filter(intersecting_cres == 0 & (cre_w_cgene == 1 | cre_w_egene == 1))

fit = glm(n ~ cre_w_cgene, family = "poisson", data = egene_cgene_atac)
summary(fit)
...
svg("plots/interval/ATAC_plot.svg", width = 4, height = 6)
...
svg("plots/interval/intersecting_ATAC_plot.svg", width = 4, height = 6)
