...
# of the egenes intersecting gold genes how many are the closest
length(unique(c(interval.distance$gene_name, eqtl.cat.distances$gene_name, sc.eqtl.distances$gene_name, gtex.distance$gene_name, mage.distance$geneSymbol)))

closest_cgene = cres_w_grnas %>%
  distinct(target_gene, .keep_all=T) %>% # Remove duplicate target-genes for different eqtl/gwas
