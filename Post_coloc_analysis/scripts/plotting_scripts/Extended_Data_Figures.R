...
cres_w_grnas %>% 
  ...
  group_by(data) %>%
  summarise(n = n()) %>%
  mutate(data = ifelse(data == "stingseq","Morris",data)) %>%
...
p1 = cres_w_grnas_egene %>%
  ...
  mutate(eqtl_study = factor(eqtl_study, levels = unique(eqtl_study)),  # Use unique values for factor levels
         group = ifelse(eqtl_study %in% c("blood QTS000019", "GTEx", "blood QTS000029", "Interval"), "Bulk_tissue_eQTL", 
...
overlap_long <- cres_w_grnas_egene %>%
  filter(target_gene %in% overlapping_cres_cgene) %>%
  mutate(
    uniq_id    = paste(eqtl, ensembl_id, grna_target, sep = "_"),
    eqtl_study = paste(eqtl, study_id, sep = "_") |> gsub("_", " ", x = _) |> trimws()
  ) %>%
  distinct(eqtl_study, ensembl_id)           # (set, item)

# Choose item type: gene IDs (ensembl_id) or triplets (uniq_id)
item_id <- "ensembl_id"

incidence <- overlap_long %>%
  transmute(item = .data[[item_id]], set = eqtl_study, present = 1L) %>%
  pivot_wider(names_from = set, values_from = present, values_fill = 0L) %>%
  distinct(item, .keep_all = TRUE) %>%
  dplyr::select(-item)

# Ensure numeric 0/1 and drop empty rows
df_up <- as.data.frame(incidence)
df_up[] <- lapply(df_up, function(x) as.integer(x > 0))
df_up   <- df_up[rowSums(df_up) > 0, , drop = FALSE]

# Order sets (columns) by size, descending (to mirror your barplot ordering)
set_order <- names(sort(colSums(df_up), decreasing = TRUE))
df_up     <- df_up[, set_order, drop = FALSE]

# Write SVG + draw UpSet
svg("plots/supp_figs/Intersecting_eGenes_per_eQTL_dataset_upset_UpSetR.svg", width = 12, height = 8)
UpSetR::upset(
  df_up,
  sets            = set_order,
  nsets           = length(set_order),
  nintersects     = NA,                      # show all non-empty intersections (set a number if too many)
  order.by = "freq",
  keep.order      = TRUE,
