...
interval.distance = fread("eQTL_gene_distances/Interval_gene_distance.txt")
...
plot.heatmap = function(x, y){
  ...
  interval.region = interval.distance %>% 
    filter(grna_target == x) %>%
  ...
  eqtl.region = rbind(eqtl.cat.region,sc.eqtl.region,gtex.region,mage.region,interval.region) %>%
  ...
  if(!"Interval" %in% rownames(heatmap_data)){
    crispri_row <- as.data.frame(t(rep(0, ncol(heatmap_data))))
    colnames(crispri_row) <- colnames(heatmap_data)
    rownames(crispri_row) = "Interval"
    heatmap_data = rbind(heatmap_data, crispri_row)
  }
  ...
  base.studies = c("CRISPRi","Interval", "CD4_T_cells", "CD8_T_cells", "B_cells","Mono_cells", "NK_cells")
  
  # Set CRISPRi & GTEX to the bottom of the heatmap
  heatmap_long$study <- factor(heatmap_long$study, 
                               levels = c("CRISPRi","Interval", "CD4_T_cells", "CD8_T_cells", "B_cells","Mono_cells","NK_cells",
  ...
  if(length(unique(heatmap_long$study)) > 9){
    ...
    svg(paste0("plots/interval/heatmaps/", gsub(":","_",x), ".svg"), width = 14, height = length(unique(heatmap_long$study))*0.5, bg = "transparent")
    ...
    plot = ggplot(heatmap_long, aes(x = gene_name, y = study, fill = pval_filled)) +
      ...
      scale_fill_gradientn(colors = c("white", "white", "red"),
                           values = scales::rescale(c(0, 1.4, 10)), na.value = "gainsboro", oob = scales::squish, limits = c(0, 10)) +  
    ...
    # Plotting the heatmap
    svg(paste0("plots/interval/heatmaps/", gsub(":","_",x), ".svg"), width = 14, height = ifelse(length(unique(heatmap_long$study)) < 4, 4, length(unique(heatmap_long$study))), bg = "transparent")
