...
# gwas_path = "data/UKBB_sumstats/30290_formatted.tsv"
# chromosome = 22
# eqtl_path = paste0("/gpfs/commons/groups/lappalainen_lab/woliveros/231005_OneK1K/analysis/02_QTL_calling/lvl1Res/NK_mean_mx/Cis_GW_allpairs/OneK1K_NK.cis_qtl_pairs.chr",chromosome,".parquet")
# eqtl_name = "NK_cells"
...
eqtl = as.data.table(read_parquet(eqtl_path))
...
eqtl[, `:=` (
  chr = as.integer(str_split_fixed(variant_id,":",4)[,1]),
  pos = as.integer(str_split_fixed(variant_id,":",4)[,2]),
  a0 = str_split_fixed(variant_id,":",4)[,3],
  a1 = str_split_fixed(variant_id,":",4)[,4])]
# x = 4,5,6
...
CHR=chromosome
hg19_lower=hg19_regions$lower[7]
hg19_upper=hg19_regions$upper[7]
...
run_coloc_susie = function(CHR, hg19_lower, hg19_upper){
  ...
  for (name in genes){
    ...
    S1 = suppressWarnings(try(runsusie(D1, estimate_residual_variance = TRUE)))
    ...
    S2 = suppressWarnings(try(runsusie(D2,estimate_residual_variance = TRUE)))
    #print(summary(S2))
    sets = S2$sets
    eqtl_sets = S1$sets
    # if no credible sets lower coverage
    if(is.null(S2$sets$cs)){
      S2 = suppressWarnings(try(runsusie(D2,coverage=0.1, estimate_residual_variance = TRUE)))
      sets = S2$sets
    }
    
    if(is.null(S1$sets$cs)){
      S1 = suppressWarnings(try(runsusie(D1, coverage=0.1, estimate_residual_variance = TRUE)))
      eqtl_sets = S1$sets
    }
...
testie = lapply(1:nrow(hg19_regions), function(x) run_coloc_susie2(chromosome, hg19_regions$lower[x], hg19_regions$upper[x]))
