...
hg19_regions_path = "/gpfs/commons/groups/lappalainen_lab/sghatan/stingseq_eqtl_overlap/data/OneK1K_LDmatrices/merged_blood_trait_regions.txt" # regions to perform fine-mapping in
...
eqtl_ld_path = "data/UKBB_LDmatrices/"
gwas_n=350473
...
# CHR=chromosome
# hg19_lower=hg19_regions$lower[35]
# hg19_upper=hg19_regions$upper[35]
...
run_coloc_susie = function(CHR, hg19_lower, hg19_upper){
  ...
  for (name in genes){
    ...
    gwas_region <- unique(gwas_sumstats[variant %in% colnames(LD_gwas2)], by = "variant")
    # Calculate varbeta and z
    ...
    if(class(S1) == "try-error" || class(S2) == "try-error" || is.null(S2$sets$cs) || is.null(S1$sets$cs)) {
      ...
      if(res$summary['PP.H4.abf'] < 0.5){
        ...
        ld_order <- colnames(LD_gwas2)
        
        markers <- data.frame(
          marker = ld_order,
          chr    = CHR,
          pos    = gwas_region$Pos[match(ld_order, gwas_region$variant)],
          z_1    = gwas_region$z[match(ld_order, gwas_region$variant)],
          z_2    = eqtl_region$z[match(ld_order, eqtl_region$variant)],
          stringsAsFactors = FALSE
        )

        snp <- coloc_snps$SNP[which.max(coloc_snps$SNP.PP.H4)]
        snp <- intersect(snp, markers$marker)[1]
      ...
      if(max(res$summary[,"PP.H4.abf"]) > 0.5){
        ...
        coloc_res = coloc_res[PP.H4.abf > 0.5]
        colnames(coloc_res)[c(2,9,10)] <- c("SNP","gwas_credible_set","eqtl_credible_set")
        colnames(coloc_snps)[1] <- "SNP"
        
        # keep only SNPs summarized in coloc_res
        coloc_snps <- coloc_snps[coloc_snps$SNP %in% coloc_res$SNP, ]
        
        # choose a top SNP for THIS gene
        snp = coloc_snps$SNP[1]
        
        df2 = filter(gwas_region, variant %in% coloc_res$SNP) %>% 
          rename(SNP = variant)
        ...
        cat("PP.H4 not significant", max(res$summary[,"PP.H4.abf"]), "\n")
  ...
  if (!is.data.frame(coloc_tr_snps) || nrow(coloc_tr_snps) == 0) return(NULL)
...
run_coloc_susie2 = purrr::possibly(run_coloc_susie, otherwise = NULL, quiet = F)
# In case some regions have no results or error out
testie <- lapply(seq_len(nrow(hg19_regions)), function(x)
  run_coloc_susie2(chromosome, hg19_regions$lower[x], hg19_regions$upper[x])
)
# Keep only non-empty data.frames
testie <- Filter(function(x) is.data.frame(x) && nrow(x) > 0, testie)

if (length(testie) == 0) {
  message("No coloc results for any region; writing empty file with header=FALSE.")
  # write an empty file (optional: skip writing instead)
  res_dir <- paste0(output_dir, eqtl_name, "/", gwas_name)
  system(paste0("bash -c 'mkdir -p ", res_dir, "'"))
  file.create(paste0(res_dir, "/chr", chromosome, "_coloc_results.txt"))
  quit(save = "no", status = 0)
}

# Robust row-bind across varying columns
t2 <- data.table::rbindlist(testie, use.names = TRUE, fill = TRUE)

t3 <- dplyr::filter(t2, !is.na(SNP)) |>
  dplyr::mutate(gwas = translated_gwas_name)

# Write results
res_dir <- paste0(output_dir, eqtl_name, "/", gwas_name)
system(paste0("bash -c 'mkdir -p ", res_dir, "'"))
data.table::fwrite(t3, paste0(res_dir, "/chr", chromosome, "_coloc_results.txt"),
                   sep = ",", quote = FALSE)
