...
cat("This script performs loads eqtl summar\n")
...
hg19_regions_path = "New_multi_coloc_pipeline/lead_loci/chr11.regions"
gwas_path = "data/UKBB_sumstats/30040_formatted.tsv"
hg38_regions_path = "data/GTEx_EU_LDmatrices/merged_hg38_blood_regions.txt"
eqtl_path = "data/eQTL_sumstats/GTEx_whlbld.sumstats"
...
hg19_regions <- fread(file = hg19_regions_path, header = F, stringsAsFactors = FALSE, fill=T) %>%
  rename(CHR = V1, lower = V2, upper = V3) %>% mutate_at(c("CHR","lower", "upper"), as.numeric) %>%
  mutate(hg19_region = paste0(CHR,":",lower,"-",upper))
...
hg38_regions = fread(hg38_regions_path) %>% rename(CHR = V1, lower = V2, upper = V3, hg19_region = V4) %>%
  mutate(CHR = gsub("chr","",CHR)) %>%
  filter(CHR == hg19_regions$CHR[1])
...
eqtl = fread(paste0(eqtl_path,".chr11.hg19.txt"))
